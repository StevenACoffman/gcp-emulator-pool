// Package gcsfake provides utilities for testing GCS, e.g. cloud
...
// GCSClient provides a fake storage client that can be customized with arbitrary fake bucket
// contents.
			...
			if len(parts) > 1 {
				if dir != parts[0] {
					...
					obj = append(
						obj,
						&storage.ObjectAttrs{
							Prefix: strings.Trim(q.Prefix, q.Delimiter) + q.Delimiter + parts[0],
						},
					)
				}
				continue
